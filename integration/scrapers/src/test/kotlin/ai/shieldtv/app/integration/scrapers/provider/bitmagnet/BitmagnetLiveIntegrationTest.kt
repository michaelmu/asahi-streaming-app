...
class BitmagnetLiveIntegrationTest {
    @Test
    fun live_movie_lookup_returns_results() = runBlocking {
        val provider = BitmagnetSourceProvider()
        ...
        assertTrue("Expected live Bitmagnet to return at least one result", results.isNotEmpty())
        assertTrue(results.all { it.providerId == "bitmagnet" })
