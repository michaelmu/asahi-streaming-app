...
class BitSearchLiveIntegrationTest {
    @Test
    fun live_movie_lookup_returns_results() = runBlocking {
        val provider = BitSearchSourceProvider()
        ...
        assertTrue("Expected live BitSearch to return at least one result", results.isNotEmpty())
        assertTrue(results.all { it.providerId == "bitsearch" })
