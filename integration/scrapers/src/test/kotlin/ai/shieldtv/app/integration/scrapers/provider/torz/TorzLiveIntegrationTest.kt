...
class TorzLiveIntegrationTest {
    @Test
    fun live_movie_lookup_returns_results() = runBlocking {
        val provider = TorzSourceProvider()
        ...
        assertTrue("Expected live Torz to return at least one result", results.isNotEmpty())
        assertTrue(results.all { it.providerId == "torz" })
