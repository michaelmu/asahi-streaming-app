...
| --- | --- | --- | --- | --- | --- |
| `asahi_color_bg_app` | `#151927` | app background | root app background, large negative space, launcher background | focused outlines, warning states, CTA fills | current `@color/asahi_bg`; `themes.xml` window/status/nav bars; `asahi_app_bg.xml`; launcher backgrounds; `MainActivity` shell root; `ScreenViewFactory.backgroundColor` |
| `asahi_color_surface_1` | `#20253A` | base panel surface | standard panels, rail containers, neutral control fills | semantic warning/success/error, hero text directly over imagery | current `@color/asahi_panel`; `asahi_panel_bg.xml`; `asahi_chip_bg.xml` default; `asahi_input_bg.xml` default; `MainActivity` sidebar; `ScreenViewFactory.panelColor` |
| `asahi_color_surface_2` | `#2A314D` | elevated panel surface | dialogs, elevated panels, secondary buttons, selected neutral containers | global page background, default focus indicator | current `@color/asahi_panel_elevated`; `asahi_panel_elevated_bg.xml`; `asahi_input_bg.xml` focused; launcher banner panel; `MainActivity` content pane; `ScreenViewFactory.panelElevatedColor` |
| `asahi_color_surface_3` | `#343D5F` | strongest neutral surface | pressed/active neutral fills, utility grouping, stronger non-primary control emphasis | app-wide default fill for all components, semantic states | current `@color/asahi_panel_soft`; button/chip neutral-active fills |
| `asahi_color_accent_primary` | `#F06B5F` | branded primary accent | primary CTAs, selected chips, branded emphasis, progress fills | body text, warning/error semantics, blanket poster tinting | current `@color/asahi_accent`; `themes.xml` `colorPrimary`; primary button focused/selected fills; `asahi_progress_fill.xml`; `ScreenViewFactory.accentColor` |
| `asahi_color_accent_secondary` | `#F3A64F` | secondary accent | progress support, supportive highlights, hero/pill highlights | primary focus ring, long-form text, destructive semantics | current `@color/asahi_accent_alt`; `themes.xml` `colorSecondary`; progress accent; `ScreenViewFactory.accentAltColor` |
...
| `asahi_color_focus` | `#F08A7E` | focus indicator | focused strokes, rings, high-confidence DPAD focus cues across controls | passive decoration, unfocused borders, semantic warning use | current `@color/asahi_focus`; button/chip/input/poster focused strokes; modal action focus; selected-button focused stroke |
| `asahi_color_warning` | `#F3C76A` | warning/caution semantic | warnings, caution copy accents, non-destructive attention states | generic branding, default CTA styling | current `@color/asahi_warning`; warning text and caution states; `ScreenViewFactory.warningColor` |
...
| `asahi_color_scrim` | `#CC11141F` | scrim/overlay | modal backdrop, overlay dimming, readability support behind layered surfaces | default panel fill, focus ring, accent usage | current `@color/asahi_scrim`; hero overlays; modal backdrop; `ScreenViewFactory` overlay scrim |
| `asahi_color_card_focus_fill_candidate` | `#141A24` | focused poster-card fill candidate | protected card framing behind art or low-art fallback states | general panel usage, CTA fills, text usage | current hardcoded poster-card darker fill; poster/source/detail card focus support |
